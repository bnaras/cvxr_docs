...
CVXR 1.8.x was a ground-up rewrite using R's S7 class system, making the R
implementation isomorphic with CVXPY. CVXR 1.9.x adds disciplined nonlinear
programming (DNLP) and more. See [What's New](whatsnew.qmd) for a
version-by-version summary, the new API, and the migration guide.
:::
...
check <- "\U2713"
d  <- tibble::tibble(
                  Solver = c("CLARABEL", "SCS", "OSQP", "HIGHS", "ECOS", "ECOS_BB", "GLPK", "GLPK_MI", "CPLEX", "GUROBI", "MOSEK", "CVXOPT", "PIQP", "SCIP", "XPRESS"),
                  `R package` = c('<a href="https://clarabel.org">clarabel</a>',
                                  ...
                                  '<a href="https://www.scipopt.org">scip</a>',
...
[Github](https://github.com/cvxgrp/CVXR). The 1.8.x series was a major
release implementing a complete rewrite using R's S7 class system,
making `CVXR` isomorphic with CVXPY and adding support for DGP, DPP, and
DQCP; the 1.9.x series adds disciplined nonlinear programming (DNLP).
More details can be found in [What's New](whatsnew.qmd).
