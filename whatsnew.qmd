...
* The NLP path is powered by automatic derivatives from the optional
  `sparsediff` package and one of the `ipopt` or `Uno` solver packages. Solver
  names are `"IPOPT"` (the automatic default when installed) and `"UNO"`. The
  `IPOPT` path returns no duals (matching CVXPY); the `UNO` path additionally
  recovers constraint duals via `dual_value()`.
