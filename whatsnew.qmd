...
- **11 solvers**: CLARABEL (default), SCS, OSQP, HiGHS, MOSEK, Gurobi,
  GLPK, GLPK_MI, ECOS, ECOS_BB, and CPLEX.
...
The `axis` parameter now uses 0-based indexing (matching CVXPY/NumPy):

| Old CVXR | New CVXR | Meaning |
|----------|----------|---------|
| `axis = 2` | `axis = 0` | Column-wise reduction |
| `axis = 1` | `axis = 1` | Row-wise reduction (unchanged) |
| `axis = NA` | `axis = NULL` | All entries |

Passing `axis = 2` now produces an informative error with migration guidance.
...
- **Removed**: CBC
- **Added**: HiGHS (LP, QP, MILP), Gurobi (LP, QP, SOCP, MIP)
...
8. Update `axis` arguments: `axis = 2` → `axis = 0`, `axis = NA` → `axis = NULL`
