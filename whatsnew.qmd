...
to be isomorphic with the released version of CVXPY (currently 1.8.2) for long-term maintainability. This
page summarizes the key changes from CVXR 1.x that may affect
users. 

_Note:_ Package authors encountering `Rmosek` issues while
submitting to CRAN should especially see [CRAN Submission Tip](#cran-submission-tip) below.
...
- **Warm-start support** for 8 solvers (OSQP, SCS, Gurobi, MOSEK, CLARABEL, PIQP, XPRESS MIP, HiGHS).
- **Element-wise matrix indexing** via `SpecialIndex`: `X[cbind(rows, cols)]`,
  `X[logical_mask]`, `X[linear_indices]` — natural R idioms for constraining
  specific entries of a matrix variable.
...
- **Added**: HiGHS (LP, QP, MILP), Gurobi (LP, QP, SOCP, MIP), CVXOPT
  (LP, SOCP), PIQP (QP), SCIP (LP, SOCP, MIP), XPRESS (LP, QP, SOCP, MIP)
...
If you encounter issues involving `Rmosek` package while submitting
your package to CRAN, just include the following code in
