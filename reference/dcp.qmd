...
Since `CVXR` 1.9.1, `get_bounds()` works on **any expression**, not just a
...
```{r, eval = utils::packageVersion("CVXR") >= "1.9.1"}
...
predicate `is_dpp()` checks compliance and, since 1.9.1, takes a `context`
