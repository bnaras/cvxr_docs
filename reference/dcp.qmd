...
of [functions](../reference/functions.qmd). Here are some examples of
`CVXR` expressions:
...
expression can be obtained using its `shape` property. `CVXR` will
...
cat("dimensions of Z:", Z@shape, "\n")
cat("dimensions of sum_entries(Z):", sum_entries(Z)@shape, "\n")
cat("dimensions of A %*% Z:", (A %*% Z)@shape, "\n")
```

Error raised for invalid dimensions.

```{r, error=TRUE}
tryCatch(A + Z, error = function(e) conditionMessage(e))
```
...
The sign of an expression is obtained via the `expr_sign` function:
...
cat("sign of x:", expr_sign(x), "\n")
cat("sign of a:", expr_sign(a), "\n")
cat("sign of abs(x):", expr_sign(abs(x)), "\n")
cat("sign of c*a:", expr_sign(c * a), "\n")
```
...
The curvature of an expression is determined using the `expr_curvature`
function:
...
cat("curvature of x:", CVXR:::expr_curvature(x), "\n")
cat("curvature of a:", CVXR:::expr_curvature(a), "\n")
cat("curvature of abs(x):", CVXR:::expr_curvature(abs(x)), "\n")
cat("curvature of sqrt(x):", CVXR:::expr_curvature(sqrt(x)), "\n")
```
...
```{r}
cat("sqrt(1 + square(x)) curvature:", CVXR:::expr_curvature(sqrt(1 + square(x))), "\n")
cat("p_norm(vstack(1, x), 2) curvature:", CVXR:::expr_curvature(p_norm(vstack(1, x), 2)), "\n")
```
...
problem. Calling [`visualize(prob)`](visualization.qmd) can help
pinpoint exactly where the DCP violation occurs by displaying the
expression tree with curvature annotations, highlighting offending
nodes in red. See the [Visualization](visualization.qmd) page for
detailed examples.

```{r}
## A non-DCP problem.
prob <- Problem(Minimize(sqrt(x)))
tryCatch(psolve(prob), error = function(e) conditionMessage(e))
```

## Bounds propagation

Since `CVXR` 1.9.0, `get_bounds()` works on **any expression**, not just a
variable, propagating interval bounds through affine maps, elementwise atoms, and
piecewise-linear atoms. It returns a `list(lower, upper)` of arrays shaped like
the expression (entries that cannot be determined come back as infinite).

```{r, eval = utils::packageVersion("CVXR") >= "1.9.0"}
x <- Variable(3, bounds = list(-1, 2))
get_bounds(x)            # the variable's own bounds: [-1, 2]
get_bounds(2 * x + 1)    # propagated through an affine map: [-1, 5]
get_bounds(abs(x))       # propagated through an atom: [0, 2]
```

## Parameterized problems (DPP)

Disciplined Parameterized Programming (DPP) is a rule set that lets `CVXR`
re-solve a parameterized problem quickly when only `Parameter` values change. The
predicate `is_dpp()` checks compliance and, since 1.9.0, takes a `context`
argument (`"dcp"`, the default, or `"dgp"`) so you can check DPP for geometric
programs as well:
...
z <- Variable(pos = TRUE)
prob <- Problem(Minimize(z), list(z >= a * b))

cat("is_dpp (dcp):", is_dpp(prob, context = "dcp"), "\n")  # product of params: not DCP-DPP
cat("is_dpp (dgp):", is_dpp(prob, context = "dgp"), "\n")  # but log-log-affine: DGP-DPP
```

See the [DPP Tutorial](../examples/dpp/tutorial.qmd) for more.
