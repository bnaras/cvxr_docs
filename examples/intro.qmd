...
```{r}
m <- cbind(value(betaHat), nnls.fit)
...
```{r, echo = FALSE, eval = params$mode %in% c("test", "save"), }
if (params$mode == "save") {
    saveRDS(list(beta = value(betaHat)), file = file.path(testdata_dir, "intro2.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
    intro_results <- readRDS(file = file.path(testdata_dir, "intro2.RDS"))
    expect_identical(value(betaHat), intro_results$beta)
}
```
...
```{r, echo = FALSE, eval = params$mode %in% c("test", "save"), }
if (params$mode == "save") {
    saveRDS(list(beta = value(betaHat)), file = file.path(testdata_dir, "intro3.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
...
read [a longer introduction](gentle-intro.qmd)
with more examples. 
