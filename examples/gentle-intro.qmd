...
```{r}
status(prob2.1)   # "optimal"
solution2.1               # optimal objective value
value(x)
value(y)
```

In general, when you apply `psolve()` to a Problem, several
things can happen:

1. `status(prob) == "optimal"`: The problem was solved. Values for
   the optimization variables were found, which satisfy all of the
   constraints and minimize/maximize the objective.

2. `status(prob) == "infeasible"`: The problem was _not_ solved
...
3. `status(prob) == "unbounded"`: The problem was _not_ solved
   ...
   optimal. In this case, `psolve()` returns `-Inf` for a
