...
# Problem solution
solution2.1 <- psolve(prob2.1)
problem_status(prob2.1)
solution2.1    # optimal objective value
value(x)
value(y)
...
if (params$mode == "save") {
    saveRDS(list(x = value(x), y = value(y)),
    ...
    expect_identical(value(x), gentle_intro$x)
    expect_identical(value(y), gentle_intro$y)
}
```
...
then returns the optimal objective value directly. After solving,
you can retrieve variable values using `value()` and the problem
status using `problem_status()`.

```{r}
problem_status(prob2.1)   # "optimal"
solution2.1               # optimal objective value
value(x)
value(y)
```
...
1. `problem_status(prob) == "optimal"`: The problem was solved. Values for
   the optimization variables were found, which satisfy all of the
   constraints and minimize/maximize the objective.

2. `problem_status(prob) == "infeasible"`: The problem was _not_ solved
...
3. `problem_status(prob) == "unbounded"`: The problem was _not_ solved
   ...
   optimal. In this case, `solve()` returns `-Inf` for a
...
```{r}
# Create a new problem based on example 1 with modified objective and extra constraint
prob2.2 <- Problem(Minimize(x^2 + y^2 + abs(x-y)),
                   c(prob2.1@constraints, y <= 1))

# Solve the modified problem
solution2.2 <- psolve(prob2.2)

# Examine the solution
problem_status(prob2.2)
solution2.2               # optimal objective value
value(x)
value(y)
```
...
```{r, error = TRUE}
prob2.3 <- Problem(Maximize(x^2 + y^2), prob2.1@constraints)
psolve(prob2.3)
```
...
```{r}
CVXR_solution3.1 <- psolve(prob3.1)
...
```{r}
prob3.2 <- Problem(prob3.1@objective, list(beta >= 0))
solution3.2 <- psolve(prob3.2)
```


```{r, echo=FALSE, out.width=700}
cvxrBetaNNLS <- value(beta)
...
additional argument here, namely adding `list(beta >= 0)` as the
constraint list, is doing the work of an entire package.
...
is [available here](/cvxr_dcp).  The set of DCP functions are
described in [Function Reference](../reference/functions.qmd).
