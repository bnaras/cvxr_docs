...
if (params$mode == "save") {
    saveRDS(list(risk = value(risk), ret = value(ret)), file = file.path(testdata_dir, "portfolio.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
    portfolio_results <- readRDS(file = file.path(testdata_dir, "portfolio.RDS"))
    expect_equal(value(risk), portfolio_results$risk, tolerance = 1e-10)
    expect_equal(value(ret), portfolio_results$ret, tolerance = 1e-10)
}
```
