...
returns $r_{ji} \sim \mbox{Uniform}(0.5,1.5)$ except for $r_{jn} = 1$,
...
returns with rows $r_j$, while `ps` is the vector of return
...
if (params$mode == "save") {
    saveRDS(list(bets = bets), file = file.path(testdata_dir, "kelly-strategy.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
    hub_results <- readRDS(file = file.path(testdata_dir, "kelly-strategy.RDS"))
    expect_identical(bets, hub_results$bets)
}
```
