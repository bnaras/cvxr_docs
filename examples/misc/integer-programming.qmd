...
`CVXR` provides constructors for the integer and boolean variables via
the parameter `integer = TRUE` or `boolean = TRUE` to the `Variable()`
function. These can be combined with `vstack` (analog of `rbind`) or
`hstack` (analog of `cbind`) to construct new expressions.
...
We can solve this problem as usual using the `HiGHS` solver and
obtain the optimal value as well as the solution.

```{r}
## psolve() returns the optimal value; value(x) extracts the solution
## (The deprecated solve()$getValue() pattern still works but warns.)
result <- psolve(problem, solver = "HIGHS")
check_solver_status(problem)
cat(sprintf("Optimal value: %.3f\n", result))
highs_solution <- value(x)
```

## Alternative Solvers

We can try other solvers and compare the solutions obtained.
`CVXR` supports several solvers for integer programs, including `HiGHS`.

```{r}
solutions <- data.frame(HiGHS = highs_solution)
...
assignment
problem](https://www.mathworks.com/help/optim/ug/office-assignments-binary-integer-programming-solver-based.html).
...
```{r}
occupy <- Variable(c(length(people), length(offices)), integer = TRUE)
```
...
```{r}
objective <- Maximize(sum_entries(PM * occupy))

constraints <- list(
    occupy >= 0,
    occupy <= 1,
    sum_entries(occupy, axis = 1) == 1,
    sum_entries(occupy, axis = 0) <= 1
)
```
...
```{r}
problem <- Problem(objective, constraints)
ecos_result <- psolve(problem, solver = "HIGHS")
check_solver_status(problem)
ecos_soln <- round(value(occupy), 0)
