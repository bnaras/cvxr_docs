...
constraints <- list(known * U == known * u_corr)
...
if (params$mode == "save") {
    ...
    expect_equal(result, tv_results$value, tolerance = 1e-10)
}
```
...
channels <- lapply(seq_len(colors), function(k) {
    ...
    constr <- known_color[, , k] * U_k ==
              known_color[, , k] * u_corr_color[, , k]
    list(var = U_k, constr = constr)
})
variables <- lapply(channels, `[[`, "var")
constraints_c <- lapply(channels, `[[`, "constr")

prob_color <- Problem(Minimize(total_variation(variables[[1]], variables[[2]], variables[[3]])),
                      constraints_c)
...
if (params$mode == "save") {
    ...
    expect_equal(result_color, tv_color_results$value, tolerance = 1e-10)
}
```
