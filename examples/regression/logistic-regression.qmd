...
specified as `X %*% beta` below, and the `value()` function
will compute its value after solving.  (Any other function of the estimate
...
if (params$mode == "save") {
    saveRDS(list(beta_res = beta_res, log_odds = log_odds, beta_log = beta_log), file = file.path(testdata_dir, "logistic-regression.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
    log_results <- readRDS(file = file.path(testdata_dir, "logistic-regression.RDS"))
    expect_equal(beta_res, log_results$beta_res, tolerance = 1e-10)
    expect_equal(beta_log, log_results$beta_log, tolerance = 1e-10)
    expect_equal(log_odds, log_results$log_odds, tolerance = 1e-10)
}
```
