...
if (params$mode == "save") {
    saveRDS(list(beta = value(beta)), file = file.path(testdata_dir, "glmnet_equivalence1.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
    g_net <- readRDS(file = file.path(testdata_dir, "glmnet_equivalence1.RDS"))
    expect_identical(value(beta), g_net$beta)
}
```
...
```{r, echo = FALSE, eval = params$mode %in% c("test", "save"), }
if (params$mode == "save") {
    saveRDS(list(beta = value(beta)), file = file.path(testdata_dir, "glmnet_equivalence2.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
