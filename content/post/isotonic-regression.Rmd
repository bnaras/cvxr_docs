...
[Isotonic regression](https://en.wikipedia.org/wiki/Isotonic_regression) is regression with monotonic constraints. There are several packages in R to fit isotonic regression models. We consider  [`isotone`](https://cran.r-project.org/package=isotone) which uses a pooled-adjacent-violators algorithm (PAVA) and active set methods to perform the fit. 

## Pituitary Data Example

[de Leeuw, Hornik and Mair](http://www.jstatsoft.org/v32/i05/)
provide an example consisting of data on the size of pituitary fissures for 11 subjects between 8 and 14 years of age.
...
## Other Approached to Tied Data

Package `isotone` provide additional methods for handling tied data besides the default `ties = "primary"` method; `ties = "secondary"` enforces equality within ties, and `ties = "tertiary"`
...
Any other arbitrary behavior can be specified for tied observations.
