...
inFile <- "cvxr_functions_data/index.rst"
```

## Functions

Here we This section of the tutorial describes the functions that can
be applied to `cvxr` expressions. `cvxr` uses the function information
in this section and
...
```{r, eval = FALSE}
expr_list = listexpr1, expr2, expr3)
...
```{r, echo = FALSE}
knitr::kable(table, "html", escape = FALSE) %>%
  kable_styling("striped") %>%
  column_spec(1, background = "#ececec") %>%
  column_spec(2, background = "#ececec") %>%
  column_spec(3, background = "#ececec") %>%
