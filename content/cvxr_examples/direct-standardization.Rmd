...
if (params$mode == "save") {
    saveRDS(list(weights = weights), file = file.path(testdata_dir, "direct-standardization.RDS"))
} else {
    cat("Testthat Results: No output is good\n")
    direct <- readRDS(file = file.path(testdata_dir, "direct-standardization.RDS"))
    expect_identical(weights, direct$weights)
}
```
...
[R Markdown](https://github.com/bnaras/cvxr_docs/blob/master/content/cvxr_examples/direct-standardization.Rmd)
