...
- The conic solver [`MOSEK`](https://www.mosek.com/) and the associated
CRAN package `Rmosek`.
- The commercial solver [`GUROBI`](https://www.gurobi.com/) that solves both 
  Quadratic and Conic programs.
- [`CPLEX`](https://www.ibm.com/analytics/cplex-optimizer) solver that handles 
both Quadratic and Conic programs, along with the associated CRAN pack `rcplex`.
...
Since these are optional, you have to install these packages yourself. __NOTE__: `MOSEK`
,`GUROBI`, and `CPLEX` require licenses to use them.
